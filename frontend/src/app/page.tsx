...
import { analyzeStock, getMarketOverview, type MarketOverview, type AnalysisResult } from "@/lib/api";
...
interface TopSignal {
  symbol: string; ticker: string; score: number; signal: "STRONG BUY" | "BUY" | "HOLD" | "SELL" | "STRONG SELL";
...
function WatchlistCard({ symbol, index, onRemove }: { symbol: string, index: number, onRemove: (s: string) => void }) {
  ...
  const removeButton = (
    <button 
      ...
      style={{ position: "absolute", top: 12, right: 12, zIndex: 10, background: "rgba(0,0,0,0.5)", border: "1px solid rgba(255,255,255,0.1)", color: "var(--text-secondary)", borderRadius: "50%", width: 24, height: 24, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", paddingBottom: 2, fontSize: 14 }}
  ...
  if (error || !data) {
    return (
      <div style={{ position: "relative", minHeight: 160, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }} className="glass-card p-4 text-center">
        {removeButton}
        <div style={{ fontSize: 16, fontWeight: 700, color: "var(--text-primary)", marginBottom: 8 }}>{symbol.replace(".NS", "").replace(".BO", "")}</div>
...
function MarketOverviewCard({ data }: { data: MarketOverview | undefined }) {
  if (!data) {
    return (
      <div className="glass-card p-5">
        <h2 style={{ fontSize: 14, fontWeight: 700, color: "var(--text-secondary)", letterSpacing: "0.08em", marginBottom: 16 }}>MARKET OVERVIEW</h2>
  ...
  return (
    <div className="glass-card p-5 space-y-4">
      <h2 style={{ fontSize: 13, fontWeight: 700, color: "var(--text-secondary)", letterSpacing: "0.08em" }}>MARKET OVERVIEW</h2>

      {/* Indices */}
      {[{ name: "NIFTY 50", d: nifty }, { name: "SENSEX", d: sensex }].map(({ name, d }) => d && (
        <div key={name} style={{ padding: "10px 14px", borderRadius: 10, background: "var(--hover-bg)" }}>
      ...
      {fii && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          {[
            ...
            <div key={label} style={{ padding: "8px 12px", borderRadius: 8, background: "var(--hover-bg)", textAlign: "center" }}>
      ...
      {breadth && (
        <div>
          <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 6, fontWeight: 600 }}>MARKET BREADTH</div>
  ...
  return (
    <div style={{ maxWidth: 1400 }}>
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 4 }}>AI-powered signals for NSE/BSE Indian equities</p>
      </motion.div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "260px 1fr 260px", gap: 20 }}>
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <h2 style={{ fontSize: 13, fontWeight: 700, color: "var(--text-secondary)", letterSpacing: "0.08em", margin: 0 }}>MY WATCHLIST</h2>
            <span style={{ fontSize: 11, color: "var(--text-muted)" }}>{mounted ? watchlist.length : 0} stocks</span>
          </div>
          {!mounted ? (
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <h2 style={{ fontSize: 13, fontWeight: 700, color: "var(--text-secondary)", letterSpacing: "0.08em", margin: 0 }}>TOP BUY SIGNALS</h2>
          ...
          {topSignalsData && (
            <div style={{ marginBottom: 10, fontSize: 10, color: topSignalsData.data_source === "precomputed" ? "#22c55e" : "#f59e0b",
              ...
              <span style={{ color: "var(--text-muted)" }}>· {topSignalsData.count} signals found</span>
            </div>
          )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {!topSignalsData ? (
              ...
              <div className="glass-card p-4" style={{ textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>
                <div style={{ fontSize: 24, marginBottom: 6 }}>🟡</div>
                No strong buy signals right now
                <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 4 }}>Next nightly scan at 4:00 PM IST</div>
              </div>
            ) : (
              topSignals.map((r, i) => (
                <motion.div
                        ...
                        {(["T", "F", "S"] as const).map((k, j) => {
                          ...
                          return (
                            <div key={k} style={{ flex: 1 }}>
                              <div style={{ fontSize: 9, color: "var(--text-muted)", marginBottom: 2 }}>{k} {vals[k]}</div>
